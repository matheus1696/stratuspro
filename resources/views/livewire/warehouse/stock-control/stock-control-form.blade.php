<div>
    <x-form.form-group>
        <div class="col-span-12 md:col-span-2">
            ...
            <x-form.form-input name="code" value="{{ old('code') ?? $dbStockcontrol->code ?? ''}}" placeholder="123456" required />
            <x-form.form-error for="code" />
        </div>
        
        <div class="col-span-12 md:col-span-6">
            <x-form.form-label for="title" value="Nome do Almoxarifado"/>
            <x-form.form-input name="title" value="{{ old('title') ?? $dbStockcontrol->title ?? ''}}" placeholder="Nome Completo" required />
            ...
            <x-form.form-select name="establishment_id" id="establishment_id">
                ...
                @foreach ($dbEstablishments as $dbEstablishment)
                    <option 
                        ...
                        {{ old('establishment_id', $dbStockcontrol->establishment_id ?? '') == $dbEstablishment->id ? 'selected' : '' }}
                    >
