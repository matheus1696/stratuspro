<div>
    <div class="bg-white rounded-lg shadow-lg p-6 text-sm">
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div><strong>Data Início:</strong> {{$dbContract->start_date}}</div>
            <div><strong>Data Fim:</strong> {{$dbContract->end_date}}</div>
        ...
        <div class="flex justify-center items-center gap-3 mt-2 border-t border-gray-300 pt-4">
            <a href="{{ route('contracts.edit', $dbContract->id) }}" class="flex flex-col justify-center items-center gap-1 bg-yellow-500 hover:bg-yellow-600 text-gray-800 hover:text-gray-900 rounded-lg shadown-lg py-3 px-5 text-xs transition-all duration-300">
