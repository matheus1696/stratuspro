<div>
    <x-pages.conteiner class="text-sm mb-5">        
        <div class="flex flex-col justify-center items-end gap-3">
            <a href="{{ route('warehouse_storages.edit', $dbWarehouseStorage->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-gray-800 hover:text-gray-900 rounded-full shadown-lg text-xs transition-all duration-300 px-2.5 py-2 absolute">
                <i class="fas fa-pen"></i>
            </a>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-6 gap-5">
            <div><strong>Código</strong> {{$dbWarehouseStorage->code}}</div>
            <div class="lg:col-span-3"><strong>Título:</strong> {{$dbWarehouseStorage->title}}</div>
            <div class="lg:col-span-2"><strong>Tipo:</strong> {{$dbWarehouseStorage->WarehouseType->title}} </div>
            <div class="lg:col-span-6"><strong>Endereço:</strong> {{$dbWarehouseStorage->CompanyEstablishment->address}}, {{$dbWarehouseStorage->CompanyEstablishment->number}}, {{$dbWarehouseStorage->CompanyEstablishment->district}}, {{$dbWarehouseStorage->CompanyEstablishment->RegionCity->title}}-{{$dbWarehouseStorage->CompanyEstablishment->RegionState->acronym}}, {{$dbWarehouseStorage->CompanyEstablishment->RegionCountry->title}}</div>
            <div class="lg:col-span-2"><strong>Latidude:</strong> {{$dbWarehouseStorage->latitude}} </div>
            <div class="lg:col-span-2"><strong>Longitude:</strong> {{$dbWarehouseStorage->longitude}} </div>
            <div class="lg:col-span-6"><strong>Unidade Vinculada:</strong> {{$dbWarehouseStorage->CompanyEstablishment->title}} </div>
        </div>
    </x-pages.conteiner>
                ...
                <div class="h-60 w-full overflow-y-auto text-sm ">
                    @foreach ($dbUsers as $dbUser)
                        <div class="flex items-center justify-between gap-2 border-b border-red-200 hover:bg-red-200 ">
                            ...
                            <a class="px-3.5 py-2 hover:text-white" href="{{ route('warehouse_storages.permission',['warehouse_storage'=>$dbWarehouseStorage->id, 'user'=>$dbUser->id]) }}">
